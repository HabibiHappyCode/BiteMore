...
import { fetchingPasta } from '../../lib/pastaData'

function PastaMeal() {

    const { data, isLoading, error, isError } = useQuery({
        queryKey: ['pasta'],
        queryFn: fetchingPasta
    })
                    ...
                    {data?.map((meal) => (
                        <div key={meal.idMeal} className="group bg-[#FFFFFF] w-[24%] max-lg:w-[32%] max-md:w-[48%] max-sm:w-[47.5%] rounded-xl overflow-hidden ease-in-out duration-200 hover:translate-y-[-10px]">
                            ...
                            <article className="px-5 max-sm:px-2 py-8 max-sm:py-5 ">
                                ...
                                <main className='mt-5 flex justify-between max-sm:justify-center items-center max-sm:flex-wrap max-sm:gap-2'>
